    ...
    def prepare_image_info(
        ...
        if by_tile:
            self.y_size, self.x_size = tile_pad_output_size(
                self.y_size, self.x_size, tile_size=tile_size
            )

        self.pyr_levels, self.pyr_shapes = get_pyramid_info(
            self.y_size, self.x_size, self.reg_image.n_ch, tile_size
        )
...
class MergeOmeTiffWriter(OmeTiffWriter):
    ...
    def write_image_by_plane(
        ...
        if len(transform_data) != len(self.reg_image.images):
            raise ValueError(
        ...
        with TiffWriter(f"{output_file_name}.ome.tiff", bigtiff=True) as tif:
            for m_idx, merge_image in enumerate(self.reg_image.images):
                ...
                for channel_idx in range(merge_n_ch):
                    ...
                    # write channel data
                    print(
                        f" writing image {m_idx} - channel {channel_idx} - shape: {image.shape}"
                    )
                    tif.write(
                        image,
                        subifds=self.subifds,
