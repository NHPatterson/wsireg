...
class RegTransformSeq:
    ...
    def set_output_spacing(
        ...
        new_output_size = np.ceil(
            np.divide(self._output_size, output_size_scaling)
        )
    ...
    def _update_transform_properties(self):
    ...
    def _build_transform_data(self):
...
def read_wsireg_transform(
    transform_fp: Union[str, Path]
) -> Tuple[List[Dict[str, List[str]]], List[int]]:

    parameter_data = json.load(open(transform_fp, "r"))
